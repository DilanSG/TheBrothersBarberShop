...
const NotificationContainer = () => {
  ...
  const getNotificationSize = (message, title) => {
    ...
    if (isMultiline || hasTitle) {
      return 'sm:max-w-md'; // Más ancho para contenido largo
    }
    return 'sm:max-w-sm'; // Compacto para mensajes cortos
  };
            ...
            <div className="flex items-start gap-2 sm:gap-3">
              ...
              <div className="flex-1 min-w-0">
                {notification.title && notification.title.trim().length > 0 && (
                  <p className="text-xs sm:text-sm font-medium text-white mb-1 truncate">
                    {notification.title}
                  </p>
                )}
                <div className={`text-xs sm:text-sm leading-tight ${(!notification.title || notification.title.trim().length === 0) ? 'font-medium' : ''}`}>
