...
export const getFrequencyDescription = (expense) => {
  // Import dinámico no funciona en exports, usar importación estática en index.js
  const config = calculator.normalizeConfig(expense);
  
  // Formatear manualmente basado en el patrón
  const { pattern, interval, config: patternConfig } = config;
