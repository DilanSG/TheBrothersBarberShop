...
class NavigationAnalytics {
  ...
  trackRouteVisit(path, userRole = null, referrer = null) {
    ...
    // console.log(`📊 Analytics: Route visited - ${path} (${userRole || 'anonymous'})`);
    return routeData;
  }
  ...
  trackRouteLoadTime(path, loadTime) {
    ...
    // console.log(`⚡ Analytics: Load time - ${path}: ${loadTime}ms`);
  }
  ...
  trackTimeSpent(path, timeSpent) {
    ...
    // console.log(`⏱️ Analytics: Time spent - ${path}: ${(timeSpent / 1000).toFixed(1)}s`);
  }
  ...
  trackInteraction(path, interactionType, target = null) {
    ...
    // console.log(`👆 Analytics: Interaction - ${path}: ${interactionType} ${target || ''}`);
  }
  ...
  trackConversion(path, conversionType, value = null) {
    ...
    // console.log(`🎯 Analytics: Conversion - ${path}: ${conversionType} (${value || 'no value'})`);
  }
