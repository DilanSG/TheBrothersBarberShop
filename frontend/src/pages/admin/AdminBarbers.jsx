...
} from '../../components/admin/DetailedModals';
...
const DayRangeFilter = ({ selectedDays, onDaysChange, selectedEndDate, onDateChange, availableDates, loading }) => {
  ...
  return (
    <div className="space-y-4">
      ...
      {showCalendarModal && console.log('📅 DEBUG CALENDARIO - availableDates:', availableDates)}
    </div>
  );
};
    ...
    <div className="group relative bg-transparent border border-white/10 rounded-2xl backdrop-blur-sm shadow-2xl shadow-blue-500/20 overflow-hidden min-h-[400px] flex flex-col">
      ...
      <div className="relative p-6 lg:p-8 flex-1 flex flex-col">
          ...
          <div className="relative flex-shrink-0 z-30">
            ...
            <button
              onClick={(e) => {
                ...
                console.log('🎯 CLICK EN FOTO DETECTADO - Navegando al perfil público del barbero:', barber.user?.name);
                console.log('🎯 BarberID:', barber._id);
              ...
              {barber.user?.profilePicture ? (
                <>
                  <img
                    ...
                    onLoad={() => {
                      console.log('✅ Profile picture loaded successfully for:', barber.user?.name);
                    }}
                  />
        ...
        {isLoading && !totals ? (
          ...
          <button
            onClick={() => {
              console.log('🚨 CLICK EN VENTAS DETECTADO');
              console.log('🚨 barberId que se va a pasar:', barber._id);
              console.log('🚨 función onSalesClick:', onSalesClick);
...
const AdminBarbers = () => {
  ...
  console.log('�🟢🟢 ADMINBARBERS COMPONENT RENDERING 🟢🟢🟢');
  ...
  useEffect(() => {
    console.log('🎯 USEEFFECT FILTRO INICIAL:', {
    ...
    if (barbers.length > 0 && !loading && selectedDayRange === 'general' && typeof applyFilter === 'function' && Object.keys(statistics).length === 0) {
      console.log('✅ APLICANDO FILTRO GENERAL INICIAL...', { 
        barbersLength: barbers.length,
      ...
      console.log('❌ NO SE APLICA FILTRO - Condiciones no cumplidas:', {
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      ...
      if (currentBarbers.length > 0 && !loading && !filterLoading && Object.keys(statistics).length === 0 && Object.keys(filteredStats).length === 0) {
        console.log('🔄 FORZANDO CARGA INICIAL DESPUÉS DE NAVEGACIÓN', { 
          barbersLength: barbers.length,
        ...
        console.log('⚠️ No hay barberos disponibles para forzar carga inicial');
      }
  ...
  console.log('🔍 AdminBarbers - Hook data:', {
  ...
  const handleDayRangeChange = async (days) => {
    console.log(`🎯 [AdminBarbers] Cambiando filtro a: ${days}`, { 
    ...
    console.log(`🎯 [AdminBarbers] Datos de barberos a usar:`, { 
    ...
    if (!activeBarbersData || activeBarbersData.length === 0) {
      console.log('⚠️ [AdminBarbers] No hay barberos disponibles, esperando...');
      setTimeout(() => {
  ...
  const handleEndDateChange = async (dateString) => {
    console.log(`🎯 [AdminBarbers] Cambiando fecha a: ${dateString}`, { 
      barbersLength: barbers?.length || 0,
      barbersDataLength: barbersData?.length || 0
    });
    ...
    // Si no hay barberos disponibles, esperar un poco y reintentar
    if (!activeBarbersData || activeBarbersData.length === 0) {
      console.log('⚠️ [AdminBarbers] No hay barberos disponibles para cambio de fecha, esperando...');
      setTimeout(() => {
        if (barbers?.length > 0 || barbersData?.length > 0) {
  ...
  const openSalesModal = async (barberId) => {
    console.log('🚨 INICIANDO openSalesModal - FUNCIÓN PERSONALIZADA');
    console.log('🚨 barberId recibido:', barberId);
    
    const barber = barbers.find(b => b._id === barberId);
    const barberName = barber?.user?.name || barber?.name || 'Barbero';
    const dateRange = formatDateRange();
    
    console.log('🛒 Abriendo modal de ventas detalladas:', { barberId, dateRange, selectedDayRange });
    console.log('🔍 Fechas calculadas para el modal:', { 
      selectedDayRange, 
      selectedEndDate, 
    ...
    console.log('📊 COMPARACIÓN CARD vs MODAL para', barberName, ':', {
    ...
    console.log('📅 Fechas finales para el backend:', { 
      startDateStr, 
      endDateStr, 
      barberId,
    ...
    try {
      ...
      console.log('🔍 COMPARACIÓN FINAL CARD vs MODAL para', barberName, ':', {
  ...
  const openServicesModal = async (barberId) => {
    ...
    console.log('✂️ Abriendo modal de cortes detallados:', { barberId, dateRange, selectedDayRange });
  ...
  const openAppointmentsModal = async (barberId) => {
    ...
    console.log('📅 Abriendo modal de citas detalladas:', { barberId, dateRange, selectedDayRange });
  ...
  const calculateTotals = (barberId) => {
    ...
    if (filterType === 'General') {
      console.log(`🔍 GENERAL FILTER DEBUG para ${barberId}:`, {
        filterType,
    ...
    if (!barberStats) {
      console.log(`⚠️ No hay estadísticas para barbero ${barberId} (filterType: ${filterType})`);
    ...
    if (filterType === 'General') {
      console.log(`📊 GENERAL TOTALS calculados para ${barberId}:`, totals);
    }
    ...
    <PageContainer>
      <div className="admin-barbers-container relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-6 space-y-8 min-h-screen overflow-y-auto">
        ...
        {barbers.length === 0 ? (
          ...
          <div className="text-center py-12 min-h-96">
            {(() => {
              console.log('🔄 ESTADO LOADING DEL FILTRO:', {
                statisticsKeysCount: Object.keys(statistics).length,
                filteredStatsKeysCount: Object.keys(filteredStats).length,
                loading,
                filterLoading,
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 lg:gap-8 min-h-96 pb-20">
            ...
            {(() => {
              if (filterType === 'General' && !window.debugShown) {
                console.log(`🎯 PRE-MAPEO - Estado completo para filtro General:`, {
                  barbersCount: barbers.length,
                  statisticsKeys: Object.keys(statistics),
                  filteredStatsKeys: Object.keys(filteredStats),
                  filterType,
            ...
            {barbers.map(barber => {
              ...
              if (filterType === 'General' && !barber._debugShown) {
                console.log(`🎨 RENDERIZANDO CARD para ${barber.user?.name}:`, {
                  totals,
                  isCardLoading,
                  hasData: !!hasData,
                  filterLoading,
                ...
                console.log(`👤 DATOS BARBERO HEADER para ${barber.user?.name}:`, {
