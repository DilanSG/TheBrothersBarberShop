...
export const toggleShowInHome = asyncHandler(async (req, res) => {
  ...
  if (showInHome === true) {
    ...
    // Excluir el servicio actual de la validación (ya que se está editando)
    const otherActiveServices = currentActiveServices.filter(s => s._id.toString() !== req.params.id);
    if (otherActiveServices.length >= 3) {
...
// @desc    Activar/Desactivar un servicio
// @route   PATCH /api/services/:id/active-status
// @access  Privado/Admin
export const toggleActiveStatus = asyncHandler(async (req, res) => {
  const { isActive } = req.body;
  
  if (typeof isActive !== 'boolean') {
    throw new AppError('El campo isActive debe ser un valor booleano', 400);
  }

  // Si se va a desactivar, también quitar del Home
  const updateData = { isActive };
  if (!isActive) {
    updateData.showInHome = false;
  }
  ...
  res.json({
    success: true,
    message: isActive ? 'Servicio activado exitosamente' : 'Servicio desactivado exitosamente',
    data: service
  });
});