...
class AppointmentService {
  static async getAvailableTimes(barberId, date) {
    ...
    const schedule = barber.availability[dayOfWeek];
    ...
    const availableSlots = this.generateTimeSlots(schedule.start, schedule.end, appointments);

    return availableSlots;
  }

  static generateTimeSlots(startTime, endTime, appointments) {
    const slots = [];
  ...
  static async completeAppointment(id, userId, userRole) {
    try {
      const appointment = await this.getAppointmentById(id);
      ...
      logger.info(`Cita ${id} completada por usuario ${userId}`);
      return appointment;
    } catch (error) {
