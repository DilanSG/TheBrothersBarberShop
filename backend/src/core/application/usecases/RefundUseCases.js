...
import InventoryLogUseCases from './InventoryLogUseCases.js';
import { refundVerificationService } from './RefundVerificationUseCases.js';

/**
 * Casos de uso para manejar reembolsos de ventas
 */
class RefundUseCases {
  ...
  static async processRefund(saleId, reason, adminCode, userId, userRole) {
    logger.info('[REFRESH] Iniciando proceso de reembolso', {
      saleId,
      reason,
      userId,
      userRole,
    ...
    if (userRole !== 'admin') {
      if (!refundVerificationService.validateCode(adminCode)) {
        logger.error('[ERROR] Código de verificación inválido para reembolso', {
          saleId,
          userId,
          userRole,
      ...
      logger.info('[SUCCESS] Usuario admin procesando reembolso - sin validación de código', {
        saleId,
        userId,
        userRole
      });
    ...
    try {
      ...
      if (sale.type === SALE_TYPES.PRODUCT && sale.productId) {
        ...
        try {
          await InventoryLogUseCases.createLog(
            'refund',
            sale.productId,
          ...
          logger.error('[ERROR] Error creando log de reembolso', logError);
        }
      }

      logger.info('[SUCCESS] Reembolso procesado exitosamente', {
        saleId: sale._id,
        productId: sale.productId,
      ...
      logger.error('[ERROR] Error procesando reembolso', {
        saleId,
        error: error.message,
        userId
      });
  ...
  static async deleteRefund(saleId, adminUserId) {
    ...
    if (sale.type === SALE_TYPES.PRODUCT && sale.productId) {
      try {
        ...
        logger.error('[ERROR] Error al reversar inventario en eliminación de reembolso', {
          saleId,
          productId: sale.productId,
    ...
    logger.info('[SUCCESS] Reembolso eliminado exitosamente', {
      saleId,
      productName: sale.productName,
  ...
  static async permanentDeleteRefund(saleId, adminUserId) {
    ...
    logger.info('[INFO] Creando log de auditoría para eliminación permanente', {
      saleId,
      productName: sale.productName,
      totalAmount: sale.totalAmount,
    ...
    logger.info('[SUCCESS] Reembolso eliminado permanentemente del sistema', {
      saleId,
